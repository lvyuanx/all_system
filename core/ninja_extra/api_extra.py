        ...
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    raise SysException(code=error_code, message=message, data=error_data)
            return wrapper
        return decorator
    
    
    def __init_subclass__(cls) -> None:
        api_wrapper_func = cls._api_response_wrapper()(cls.api)
...
class NinjaAPIExtra:
    ...
    def get_code_and_msg(self, code: str | Tuple[str, str], api_code: str, code_dict: dict):
        """
        获取状态码和消息
        """
        if not code:
                ...
                "content": {
                    "application/json": {
                        "example": ErrorResponse(
                            ...
                            data=data_util.generate_example(api_class.error_response_schema)
                        ).dict()
                    }
                }
            }
    ...
    def set_api_attr(self, api_class: BaseApi, api_code: str, router_code: str):
        ...
        # 处理最终异常码
        finally_code, finally_msg = self.get_code_and_msg(api_class.finally_code, api_code, code_dict)
    ...
    def register(self):
        ...
        for router in apis: # 遍历路由
            assert isinstance(router, tuple) and len(router) == 4, f"接口组配置格式错误: {router}"
            ...
            for api_group_prefix, api_group in router_apis.items():
                for api in api_group: # 遍历API
                    assert isinstance(api, tuple) and (4 <= len(api) <= 5), f"接口配置格式错误: {api}"

                    if len(api) == 4: # 没有接口配置
                        api_code, api_url, api_class, api_desc = cast(tuple[str, str, BaseApi, str], api)
                        api_conf = {}
                    else: # 携带接口配置
                        api_code, api_url, api_class, api_desc, api_conf = cast(tuple[str, str, BaseApi, str, dict], api)
