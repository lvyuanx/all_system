...
"""
# File       : order_list_by_pattern_view.py
# Description: 按版号分页查询关联订单
"""
...
class Pagination(AsyncLimitOffsetPagination):
    ...
    async def aprocess_result(self, results: list) -> list:
        for item in results:
            item["order_status_str"] = OrderStatusChoices(item["order_status"]).label if item.get("order_status") else ""
        return results
